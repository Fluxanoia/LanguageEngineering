...
fmt1 e = 
    case e of
      ...
      If e1 e2 e3      -> "if " ++ fmt1 e1 ++ " then " ++ fmt1 e2 ++ " else " ++ fmt1 e3
...
fmt2 ctxpre e = 
    case e of 
      CstI i           -> show i
      Var x            -> x 
      If e1 e2 e3      -> "if " ++ fmt2 (-1) e1 ++ " then " ++ fmt2 (-1) e2 ++ " else " ++ fmt2 (-1) e3
...
closedin (If e1 e2 e3) vs
    = closedin e1 vs && closedin e2 vs && closedin e3 vs
...
freevars (If e1 e2 e3)
    = union (union (freevars e1, freevars e2), freevars e3)
...
nsubst (If e1 e2 e3) env
    = If (nsubst e1 env) (nsubst e2 env) (nsubst e3 env)
