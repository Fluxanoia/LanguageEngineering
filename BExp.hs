module BExp where
    ...
    trans_bexp :: Bexp -> Code
    trans_bexp TRUE        = AM_True:[]
    trans_bexp FALSE       = AM_False:[]
    trans_bexp (Neg b)     = (trans_bexp b) ++ AM_Neg:[]
    trans_bexp (And b1 b2) = (trans_bexp b2) ++ (trans_bexp b1) ++ AM_And:[]
    trans_bexp (Eq a1 a2)  = (trans_aexp a2) ++ (trans_aexp a1) ++ AM_Eq:[]
    trans_bexp (Leq a1 a2) = (trans_aexp a2) ++ (trans_aexp a1) ++ AM_Leq:[]
